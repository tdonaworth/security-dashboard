        ...
        try:
            ...
            with sql.connect("latestDatabase.db") as con:
                try:
                    ...
                    cur.execute(
                        "REPLACE INTO NEXUS_LATEST_RESULTS (docker_tag,service_name,jenkins_url,nexusiq_url,yarn_log) \
                        VALUES (?,?,?,?,?)",
                        (docker_tag, service_name, jenkins_url, nexusiq_url, yarn_log),
                    )
...
def listLatest():
    ...
    cur = con.cursor()
    cur.execute(
        "select * from NEXUS_LATEST_RESULTS where docker_tag in ('latest', 'master') order by docker_tag ASC, service_name ASC"
    )
